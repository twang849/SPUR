...
export default function FoxTherapist() {
  const router = useRouter();
  const [messages, setMessages] = useState<
    { text: string; sender: "user" | "bot" }[]
  >([
    {
      text: "Hello, I'm Fox. There's always a clever way forward. What brings you here?",
      sender: "bot",
    },
  ]);
  ...
  const speak = async (text: string) => {
    setBotStatus("speaking");
    const voiceId = "D38z5RcWu1voky8WS1ja"; // Fin's voice for Fox
  ...
  function botReply(userText: string) {
    ...
    let reply = "Interesting. Tell me more about your strategy.";
    if (/hello|hi|hey/.test(lower)) {
      reply = "Greetings. What puzzle are we solving today?";
    } else if (/stuck|confused|lost/.test(lower)) {
      reply = "Let's look at this from a different angle. There's always a way.";
    } else if (/thank/.test(lower)) {
      reply = "You're welcome. Stay sharp.";
    }
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <div className={`animal-emoji-large ${botStatus}`} aria-label="Fox emoji">
          🦊
        </div>
        <div className="chatbot-container" role="region" aria-live="polite" aria-label="Chatbot conversation">
          ...
          <div className="typing-indicator">
            {botStatus === 'thinking' && 'Fox is scheming...'}

...
export default function ElephantTherapist() {
  ...
  const [sessionId] = useState(() => `session_elephant_${Date.now()}`);
  ...
  const speak = async (text: string) => {
    setBotStatus("speaking");
    const voiceId = "Yko7PKHZNXotIFUBG7I9"; // Matthew's voice for Elephant
  ...
  function botReply(userText: string) {
    ...
    let reply = "I understand. Please, continue when you are ready.";
    if (/hello|hi|hey/.test(lower)) {
      reply = "Greetings. I am here to listen with patience.";
    } else if (/sad|grief|pain/.test(lower)) {
      reply = "It is okay to feel that way. These things take time. We will get through it together.";
    } else if (/thank/.test(lower)) {
      reply = "You are most welcome. Remember your strength.";
    }
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <div className={`animal-emoji-large ${botStatus}`} aria-label="Elephant emoji">
          🐘
        </div>
        <div className="chatbot-container" role="region" aria-live="polite" aria-label="Chatbot conversation">
          ...
          <div className="typing-indicator">
            {botStatus === 'thinking' && 'Elephant is reflecting...'}
